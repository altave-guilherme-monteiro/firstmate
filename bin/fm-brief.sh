...
SEMGREP_NOTE_SHIP="Before you report this task done, run \`semgrep --config=p/security-audit --config=p/secrets\` (read-only, no \`--autofix\`); it scans the whole worktree against no baseline, so read the output yourself and weigh only the WARNING and ERROR findings that touch the files and lines you changed, using judgment to skip pre-existing findings your work did not introduce. Fix those or explicitly justify them wherever this task's result is written up; this is a suggestion, not an enforced gate. If \`semgrep\` is not installed, note it and move on - nothing in this task depends on it."
