...
On each step's first observed transition into a terminal status (`completed`, `failed`, `cancelled`, or `skipped`, of which `failed` and `cancelled` are emitted with an `error` status code and `completed` and `skipped` with an `ok` one), it emits one span named `phase:<step>` on service `firstmate-pipeline`, parented directly under the task's root carrier - so every phase span is a sibling child of the same firstmate-minted identity, and an observer sees the whole run as one stitched trace.
The poller exits once the `run:` block's own `status:` line reads `completed`, `failed`, or `cancelled`, or a top-level `outcome:` line reads `passed`, `failed`, or `cancelled`; on that closing poll every step not already seen terminal gets its span emitted with the run outcome's status code (`error` for a failed or cancelled run, `ok` otherwise), since the terminal status output carries no steps table, so every phase observed at least once during the run has exactly one span by the time the poller exits; a phase that never appeared in any steps table gets no span rather than a fabricated one.
The poller also exits, having emitted whatever it saw, after `<max-runtime-seconds>` (default 3600) as a safety bound against a wedged pipeline, or after five consecutive polls (`FM_PIPELINE_TRACE_MAX_EMPTY_POLLS`, default 5; a non-positive-integer value falls back to that default) whose output carries neither a steps table nor a run status - the shape of a `no-mistakes` that is missing, erroring, or has no run to report.
...
The primary timing source is the pipeline's own `duration_ms` column: when a step is first seen terminal with a `duration_ms` of at least one second, its span ends at that observation and starts that many whole seconds earlier, so the span length is the pipeline's own measurement (truncated to whole seconds) rather than an observation artifact.
Only the transition detection itself is poll-bounded: the span's placement on the wall clock can be late by up to one poll interval, and when the `duration_ms` column is absent from the header, or its value is under 1000ms - including every span closed at run termination - the span falls back to the interval since the step was first observed.
