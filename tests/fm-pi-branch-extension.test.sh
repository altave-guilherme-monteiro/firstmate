...
# Seed the canonical record that one merge was already published to this home's
# supervision destination, through the exact owner function
# bin/fm-merge-outcome-lib.sh calls on the real merge path
# (bin/fm-pr-lib.sh's fm_pr_poll_merge_mark_notified). No hand-written marker
# bytes: if that owner's format moves, this seeding moves with it.
seed_published_merge() {  # <home> <task-id> <pr-url>
  bash -eu -c '
    . "$1/bin/fm-pr-lib.sh"
    fm_pr_url_parse "$4" || exit 1
    fm_pr_poll_merge_mark_notified "$2/state" "$3" \
      "$FM_PR_PROVIDER" "$FM_PR_HOST" "$FM_PR_PATH" "$FM_PR_NUMBER"
  ' seed-published-merge "$ROOT" "$1" "$2" "$3"
}

# A supervision outcome the captain has NOT been given must always reach him.
# Losing one is the failure this delivery path exists to prevent, and it is
# silent: nothing announces an outcome that was never relayed. The duplicate
# suppression below therefore has to be identity-scoped, so an unrelated
# already-published merge sitting in the same home cannot swallow it.
test_new_captain_outcome_is_always_relayed_once() {
  local repo home out status kind body
  repo="$TMP_ROOT/new-outcome-root"
  home="$TMP_ROOT/new-outcome-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  seed_published_merge "$home" task-other https://github.com/example/repo/pull/41 \
    || fail "could not seed the unrelated published merge"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
if (!dispatch("signal: task-9 merge landed").accepted) throw new Error("branch did not accept the wake");
await settle(() => (globalThis.__fmPrompts ?? []).length === 1, "branch wake prompt");
const session = globalThis.__fmSessions[0];
const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");
...
const result = await report.execute(
  "new-outcome",
  {
    task: "task-9",
    verdict: "captain",
    summary: "PR https://github.com/example/repo/pull/42 merged cleanly, all checks green",
  },
  undefined,
  undefined,
  {},
);
if (result.isError) throw new Error(`captain report failed: ${JSON.stringify(result)}`);
if (sentToMain.length !== 1) throw new Error(`expected one merged note, got ${sentToMain.length}`);
const delivered = sentToMain[0];
if (delivered.options.triggerTurn !== true || delivered.options.deliverAs !== "followUp") {
  throw new Error(`a new captain outcome was not relayed as one follow-up turn: ${JSON.stringify(delivered.options)}`);
}
if (delivered.message.display !== false) throw new Error("a relayed captain outcome must not also be printed");
if (!delivered.message.content.includes("task-9: PR https://github.com/example/repo/pull/42")) {
  throw new Error(`captain note lost its outcome: ${delivered.message.content}`);
}
writeFileSync(`${home}/state/delivered-new-outcome`, delivered.message.content);
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "a genuinely new supervision outcome must still reach the captain exactly once: $out"
  kind=$(./bin/fm-operational-input.sh kind < "$home/state/delivered-new-outcome") \
    || fail "a new captain outcome lost the self-description that stops main mistaking it for its own answer"
  [ "$kind" = branch-outcome ] \
    || fail "a new captain outcome was delivered as kind '$kind', not branch-outcome"
  body=$(./bin/fm-operational-input.sh body < "$home/state/delivered-new-outcome") \
    || fail "a new captain outcome carries no readable body"
  case "$body" in
    *"relay only this outcome to the captain now"*) ;;
    *) fail "a new captain outcome was never ordered relayed: $body" ;;
  esac
  pass "a supervision outcome the captain has not been given is still relayed once, even beside an unrelated published merge"
}

# The duplicate the captain actually saw: one merge, two independently woken
# actors, both reporting it. Main is woken on its own merge poll and the branch
# is woken again by the same merge landing in a task's status log; main's text
# only reaches the branch's mirror at main's turn end, so the branch can decide
# to escalate before it could see the captain had already been told. Suppression
# therefore has to come from the canonical record, not from the mirror.
test_already_published_merge_is_downgraded_without_a_second_turn() {
  local repo home out status
  repo="$TMP_ROOT/published-merge-root"
  home="$TMP_ROOT/published-merge-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  seed_published_merge "$home" task-9 https://github.com/example/repo/pull/42 \
    || fail "could not seed the published merge"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { dispatch, settle, outcomeScript, sentToMain, home }; })()`);
const { dispatch, settle, outcomeScript, sentToMain, home } = globalThis.__t;
import { readFileSync, writeFileSync } from "node:fs";

if (!dispatch("signal: task-9 status merge landed").accepted) throw new Error("branch did not accept the wake");
await settle(() => (globalThis.__fmPrompts ?? []).length === 1, "branch wake prompt");
const session = globalThis.__fmSessions[0];
const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");

// The same merge the captain was already told about, reported by the branch
// from its own independent wake.
const duplicate = await report.execute(
  "already-published",
  {
    task: "task-9",
    verdict: "captain",
    summary: "PR https://github.com/example/repo/pull/42 has now merged cleanly (squash merge abc1234); the local copy is synced",
    wake: "signal: task-9 status",
  },
  undefined,
  undefined,
  {},
);
if (duplicate.isError) throw new Error(`duplicate report failed: ${JSON.stringify(duplicate)}`);
if (sentToMain.length !== 1) throw new Error(`expected one merged note, got ${sentToMain.length}`);
const note = sentToMain[0];
if (note.options.triggerTurn) {
  throw new Error("an already-reported merge opened a second captain turn");
}
if (note.options.deliverAs) {
  throw new Error(`an already-reported merge was still queued as ${note.options.deliverAs}`);
}
if (note.message.display !== true) {
  throw new Error("the deduplicated outcome was hidden instead of appended as an ordinary note");
}
if (typeof note.message.content !== "string" || !note.message.content.startsWith("⛵ ")) {
  throw new Error(`the deduplicated outcome is not the ordinary rendered note: ${note.message.content}`);
}
if (!note.message.content.includes("task-9: PR https://github.com/example/repo/pull/42")) {
  throw new Error(`the deduplicated outcome lost its content: ${note.message.content}`);
}

// A different merge in the same task is a different event and must still reach
// the captain: suppression is bound to the canonical PR identity, not to the
// task or to having reported anything at all.
const fresh = await report.execute(
  "not-yet-published",
  {
    task: "task-9",
    verdict: "captain",
    summary: "PR https://github.com/example/repo/pull/43 merged cleanly, all checks green",
  },
  undefined,
  undefined,
  {},
);
if (fresh.isError) throw new Error(`follow-on report failed: ${JSON.stringify(fresh)}`);
if (sentToMain.length !== 2) throw new Error(`expected two merged notes, got ${sentToMain.length}`);
if (sentToMain[1].options.triggerTurn !== true || sentToMain[1].options.deliverAs !== "followUp") {
  throw new Error(`a merge the captain had not been told about was suppressed too: ${JSON.stringify(sentToMain[1].options)}`);
}
if (sentToMain.filter((sent) => sent.options.triggerTurn).length !== 1) {
  throw new Error("two independent notifications of one merge still cost two captain turns");
}

// The branch's own verdict is recorded as it was decided: the downgrade is a
// delivery choice, not a rewrite of what the branch judged, and both rows are
// still marked read so neither replays at the next session start.
const rows = readFileSync(`${home}/state/branch-outcomes.jsonl`, "utf8").trim().split("\n").map((line) => JSON.parse(line));
if (rows.length !== 2 || rows.some((row) => row.verdict !== "captain")) {
  throw new Error(`the outcome store did not keep both captain verdicts: ${JSON.stringify(rows)}`);
}
if (outcomeScript(["unread"]) !== "") throw new Error("a downgraded outcome was left unread");
writeFileSync(`${home}/state/delivered-deduplicated-note`, note.message.content);
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "a merge the captain was already told about must not be reported again or cost a second turn: $out"
  # The deduplicated note takes the rendered path, whose renderer reads the
  # glyph off the front of this exact string, so it must be plain text rather
  # than a typed envelope addressed to main's model.
  if ./bin/fm-operational-input.sh kind < "$home/state/delivered-deduplicated-note" >/dev/null 2>&1; then
    fail "the deduplicated outcome was still delivered as instructed input to main's model"
  fi
  pass "a merge the captain was already told about is appended once as an ordinary note and opens no second turn"
}
