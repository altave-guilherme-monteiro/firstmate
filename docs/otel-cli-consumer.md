...
- `fm_otel_wrap <traceparent> <service> <name> -- <command...>` - runs `<command...>` unconditionally, times it, emits a span for it when emission is enabled, and always returns the wrapped command's own exit status verbatim, whether or not the span was emitted.
