...
The poller also exits, having emitted whatever it saw, after `<max-runtime-seconds>` (default 3600) as a safety bound against a wedged pipeline, or after five consecutive polls whose output carries neither a steps table nor a run status - the shape of a `no-mistakes` that is missing, erroring, or has no run to report.
